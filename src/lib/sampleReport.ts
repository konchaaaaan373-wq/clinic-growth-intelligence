...
export const SAMPLE_REPORT: AuditReport = {
  ...
  scores: {
    ...
    medicalAdRisk: {
      ...
      label: "医療広告リスク",
      explanation:
        "文脈によっては確認が望ましい表現を機械的に初期スクリーニングし、文脈に応じて優先確認/要確認/文脈確認に分類します。分類は人が確認する際の優先順位であり、法的判断・法的リスクの評価ではなく、単語検出のみで違反を断定しません。",
      positives: ["保証・最上級を断定するような優先確認にあたる表現は検出されませんでした"],
      negatives: [
        "要確認 2 件（違反の断定ではありません）",
        "【要確認】「最新」— 文脈により確認が望ましい可能性があります",
        "【要確認】「痛くない」— 文脈により確認が望ましい可能性があります",
      ],
    },
    mmmReadiness: {
      score: 9,
