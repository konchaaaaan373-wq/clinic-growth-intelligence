...
export const SAMPLE_REPORT: AuditReport = {
  ...
  scores: {
    ...
    medicalAdRisk: {
      ...
      label: "医療広告リスク",
      explanation:
        "文脈によっては確認が望ましい表現を機械的に初期スクリーニングし、文脈に応じて優先確認/要確認/文脈確認に分類します。法的判断ではなく、単語検出のみで違反を断定しません。",
      positives: ["保証・最上級を断定するような高リスク表現は検出されませんでした"],
      negatives: [
        "要確認 2 件・文脈確認 0 件（違反の断定ではありません）",
